<?php
require ('mes_classes/Database.php');
require ('mes_classes/Vente.php');

class Fonction{

    // Total ventes du jour
    public function totaliteVentes($conn, $date){
        $query = "SELECT SUM(montant) AS total FROM ventes WHERE date_vente = :date";
        $stmt = $conn->prepare($query);
        $stmt->execute([':date' => $date]);
        return $stmt->fetchColumn();
    }

    // Avance du jour
    public function totaliteAvance($conn, $date){
        $query= "SELECT SUM(avance) AS total FROM ventes WHERE date_vente = :date";
        $stmt= $conn->prepare($query);
        $stmt->execute([':date' => $date]);
        return $stmt->fetchColumn();
    }

    // Restant du jour
    public function totaliteRestant($conn, $date){
        $query= "SELECT SUM(reste) AS total FROM ventes WHERE date_vente = :date";
        $stmt= $conn->prepare($query);
        $stmt->execute([':date' => $date]);
        return $stmt->fetchColumn();
    }

    // Dépenses du jour
    public function addDepance($conn, $date){
        $query= "SELECT SUM(montant) AS total FROM depenses WHERE date_depense = :date";
        $stmt= $conn->prepare($query);
        $stmt->execute([':date' => $date]);
        return $stmt->fetchColumn();
    }

    // Paiements reçus aujourd'hui (DETTE payée aujourd'hui)
    public function dettespayer($conn, $date){
        $query= "SELECT SUM(p.montant) AS total 
                 FROM paiements p
                 INNER JOIN ventes v ON p.id_vente = v.id  
                 WHERE p.date_paiement = :date";
        $stmt= $conn->prepare($query);
        $stmt->execute([':date' => $date]);
        return $stmt->fetchColumn();
    }

    // Anciennes dettes (paiements reçus aujourd'hui mais ventes d'avant)
    public function anciennesDettes($conn, $date){
        $query= "SELECT SUM(p.montant) AS total 
                 FROM paiements p
                 INNER JOIN ventes v ON p.id_vente = v.id  
                 WHERE p.date_paiement = :date 
                 AND v.date_vente <> :date"; 
        $stmt= $conn->prepare($query);
        $stmt->execute([':date' => $date]);
        return $stmt->fetchColumn();
    }
}




require ('../mes_classes/database.php');
require ('../mes_classes/Vente.php');
require ('../mes_classes/fonction.php');

$db = new Database();
$connexion = $db->getConnection();

$date = $_GET['date'] ?? date('Y-m-d');


$donner1 = new Vente($connexion,null,null,null,null,null,);

$vente_jours = $donner1->getventes($connexion,$connexion);


$donneer2 = new Fonction();

// Récupération des données sous forme de nombres
$totaux_ventes   = $donneer->totaliteVentes($connexion, $date)[0]['total'] ?? 0;
$totaux_avance   = $donneer->totaliteAvance($connexion, $date)[0]['avance'] ?? 0;
$totaux_restant  = $donneer->totaliteRestant($connexion, $date)[0]['reste'] ?? 0;

$totalDepenses   = $donneer->addDepance($connexion, $date)[0]['depense'] ?? 0;

$dette_jour      = $donneer->dettespayer($connexion, $date)[0]['dette_jour'] ?? 0;
$ancienne_dettes = $donneer->anciennesDettes($connexion, $date)[0]['anciennes'] ?? 0;



// Total encaissé = avances + dette du jour + anciennes dettes
$encaissements = $totaux_avance + $dette_jour + $ancienne_dettes;

// Solde final
$solde = $encaissements - $totalDepenses;

// Charger la vue
include ('../views/bilan.php');

?>

<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Bilan Journalier</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css">
</head>

<body class="bg-light">

<div class="container mt-4">

    <h2 class="text-center mb-4">
        📊 Bilan du <?= date("d/m/Y", strtotime($date)) ?>
    </h2>

    <!-- Sélecteur de date -->
    <form method="GET" class="mb-3" method="../controllers/controllerBilan.php">
        <div class="row">
            <div class="col-md-4 offset-md-3">
                <input type="date" name="date" value="<?= $date ?>" class="form-control">
            </div>
            <div class="col-md-2">
                <button class="btn btn-primary w-100">Afficher</button>
            </div>
        </div>
    </form>


    <!-- ===========================================================
         SECTION : TOTALS GENERAUX
    ============================================================ -->
    <div class="card p-4 shadow mb-4">
        <h4 class="mb-3">Récapitulatif général</h4>
        <table class="table table-bordered">
            <tr>
                <th>Total des ventes du jour :</th>
                <td><?= $totaux_ventes ?> FCFA</td>
            </tr>

            <tr>
                <th>Avances reçues aujourd’hui :</th>
                <td><?= $totaux_avance ?> FCFA</td>
            </tr>

            <tr>
                <th>Restes du jour :</th>
                <td><?= $totaux_restant ?> FCFA</td>
            </tr>

            <tr class="table-warning">
                <th>Dette payée aujourd’hui :</th>
                <td><?= $dette_jour ?> FCFA</td>
            </tr>

            <tr class="table-warning">
                <th>Anciennes dettes encaissées :</th>
                <td><?= $ancienne_dettes ?> FCFA</td>
            </tr>

            <tr class="table-danger">
                <th>Total des dépenses :</th>
                <td><?= $totalDepenses ?> FCFA</td>
            </tr>

            <tr class="table-info">
                <th>Total encaissé aujourd’hui :</th>
                <td><strong><?= $encaissements ?> FCFA</strong></td>
            </tr>

            <tr class="table-success">
                <th>Solde final :</th>
                <td><strong><?= $solde ?> FCFA</strong></td>
            </tr>
        </table>
    </div>



    <!-- ===========================================================
         SECTION : VENTES DU JOUR
    ============================================================ -->
    <div class="card p-4 shadow mb-4">
        <h4>📦 Ventes du jour</h4>

        <?php if (!empty($vente_jours)): ?>
            <table class="table table-striped table-bordered mt-3">
                <thead>
                    <tr>
                        <th>Client</th>
                        <th>Poisson</th>
                        <th>Poids (kg)</th>
                        <th>Total</th>
                        <th>Avance</th>
                        <th>Reste</th>
                        <th>Date Vente</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($vente_jours as $v): ?>
                    <tr>
                        <td><?= $v['nom_client'] ?></td>
                        <td><?= $v['poisson'] ?></td>
                        <td><?= $v['poids'] ?></td>
                        <td><?= $v['total'] ?> FCFA</td>
                        <td><?= $v['avance'] ?> FCFA</td>
                        <td><?= $v['reste'] ?> FCFA</td>
                        <td><?= date("d/m/Y", strtotime($v['date_vente'])) ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        <?php else: ?>
            <p class="text-muted">Aucune vente enregistrée.</p>
        <?php endif; ?>
    </div>



    <!-- ===========================================================
         SECTION : DETTES PAYÉES (du jour + anciennes)
    ============================================================ -->
    <div class="card p-4 shadow mb-4">
        <h4>💰 Paiements reçus</h4>

        <?php if (!empty($paiements_jour)): ?>
            <table class="table table-striped table-bordered mt-3">
                <thead>
                    <tr>
                        <th>Client</th>
                        <th>Montant payé</th>
                        <th>Type</th>
                        <th>Date Paiement</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($paiements_jour as $p): ?>
                    <tr>
                        <td><?= $p['nom_client'] ?></td>
                        <td><?= $p['montant'] ?> FCFA</td>
                        <td>
                            <?php if ($p['date_vente'] == $date): ?>
                                <span class="badge bg-warning">Dette du jour</span>
                            <?php else: ?>
                                <span class="badge bg-info">Ancienne dette</span>
                            <?php endif; ?>
                        </td>
                        <td><?= date("d/m/Y", strtotime($p['date_paiement'])) ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        <?php else: ?>
            <p class="text-muted">Aucun paiement enregistré aujourd'hui.</p>
        <?php endif; ?>
    </div>



    <!-- ===========================================================
         SECTION : DÉPENSES
    ============================================================ -->
    <div class="card p-4 shadow mb-4">
        <h4>📉 Dépenses du jour</h4>

        <?php if (!empty($depenses_jour)): ?>
            <table class="table table-striped table-bordered mt-3">
                <thead>
                    <tr>
                        <th>Motif</th>
                        <th>Montant</th>
                        <th>Date</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($depenses_jour as $d): ?>
                    <tr>
                        <td><?= $d['motif'] ?></td>
                        <td><?= $d['montant'] ?> FCFA</td>
                        <td><?= date("d/m/Y", strtotime($d['date'])) ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>

        <?php else: ?>
            <p class="text-muted">Aucune dépense enregistrée aujourd'hui.</p>
        <?php endif; ?>
    </div>

</div>

</body>
</html>
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Bilan Journalier</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css">
</head>

<body class="bg-light">

<div class="container mt-4">

    <h2 class="text-center mb-4">
        📊 Bilan du <?= date("d/m/Y", strtotime($date)) ?>
    </h2>

    <!-- Sélecteur de date -->
    <form method="GET" class="mb-3" method="../controllers/controllerBilan.php">
        <div class="row">
            <div class="col-md-4 offset-md-3">
                <input type="date" name="date" value="<?= $date ?>" class="form-control">
            </div>
            <div class="col-md-2">
                <button class="btn btn-primary w-100">Afficher</button>
            </div>
        </div>
    </form>


    <!-- ===========================================================
         SECTION : TOTALS GENERAUX
    ============================================================ -->
    <div class="card p-4 shadow mb-4">
        <h4 class="mb-3">Récapitulatif général</h4>
        <table class="table table-bordered">
            <tr>
                <th>Total des ventes du jour :</th>
                <td><?= $totaux_ventes ?> FCFA</td>
            </tr>

            <tr>
                <th>Avances reçues aujourd’hui :</th>
                <td><?= $totaux_avance ?> FCFA</td>
            </tr>

            <tr>
                <th>Restes du jour :</th>
                <td><?= $totaux_restant ?> FCFA</td>
            </tr>

            <tr class="table-warning">
                <th>Dette payée aujourd’hui :</th>
                <td><?= $dette_jour ?> FCFA</td>
            </tr>

            <tr class="table-warning">
                <th>Anciennes dettes encaissées :</th>
                <td><?= $ancienne_dettes ?> FCFA</td>
            </tr>

            <tr class="table-danger">
                <th>Total des dépenses :</th>
                <td><?= $totalDepenses ?> FCFA</td>
            </tr>

            <tr class="table-info">
                <th>Total encaissé aujourd’hui :</th>
                <td><strong><?= $encaissements ?> FCFA</strong></td>
            </tr>

            <tr class="table-success">
                <th>Solde final :</th>
                <td><strong><?= $solde ?> FCFA</strong></td>
            </tr>
        </table>
    </div>



    <!-- ===========================================================
         SECTION : VENTES DU JOUR
    ============================================================ -->
    <div class="card p-4 shadow mb-4">
        <h4>📦 Ventes du jour</h4>

        <?php if (!empty($vente_jours)): ?>
            <table class="table table-striped table-bordered mt-3">
                <thead>
                    <tr>
                        <th>Client</th>
                        <th>Poisson</th>
                        <th>Poids (kg)</th>
                        <th>Total</th>
                        <th>Avance</th>
                        <th>Reste</th>
                        <th>Date Vente</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($vente_jours as $v): ?>
                    <tr>
                        <td><?= $v['nom_client'] ?></td>
                        <td><?= $v['poisson'] ?></td>
                        <td><?= $v['poids'] ?></td>
                        <td><?= $v['total'] ?> FCFA</td>
                        <td><?= $v['avance'] ?> FCFA</td>
                        <td><?= $v['reste'] ?> FCFA</td>
                        <td><?= date("d/m/Y", strtotime($v['date_vente'])) ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        <?php else: ?>
            <p class="text-muted">Aucune vente enregistrée.</p>
        <?php endif; ?>
    </div>



    <!-- ===========================================================
         SECTION : DETTES PAYÉES (du jour + anciennes)
    ============================================================ -->
    <div class="card p-4 shadow mb-4">
        <h4>💰 Paiements reçus</h4>

        <?php if (!empty($paiements_jour)): ?>
            <table class="table table-striped table-bordered mt-3">
                <thead>
                    <tr>
                        <th>Client</th>
                        <th>Montant payé</th>
                        <th>Type</th>
                        <th>Date Paiement</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($paiements_jour as $p): ?>
                    <tr>
                        <td><?= $p['nom_client'] ?></td>
                        <td><?= $p['montant'] ?> FCFA</td>
                        <td>
                            <?php if ($p['date_vente'] == $date): ?>
                                <span class="badge bg-warning">Dette du jour</span>
                            <?php else: ?>
                                <span class="badge bg-info">Ancienne dette</span>
                            <?php endif; ?>
                        </td>
                        <td><?= date("d/m/Y", strtotime($p['date_paiement'])) ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        <?php else: ?>
            <p class="text-muted">Aucun paiement enregistré aujourd'hui.</p>
        <?php endif; ?>
    </div>



    <!-- ===========================================================
         SECTION : DÉPENSES
    ============================================================ -->
    <div class="card p-4 shadow mb-4">
        <h4>📉 Dépenses du jour</h4>

        <?php if (!empty($depenses_jour)): ?>
            <table class="table table-striped table-bordered mt-3">
                <thead>
                    <tr>
                        <th>Motif</th>
                        <th>Montant</th>
                        <th>Date</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($depenses_jour as $d): ?>
                    <tr>
                        <td><?= $d['motif'] ?></td>
                        <td><?= $d['montant'] ?> FCFA</td>
                        <td><?= date("d/m/Y", strtotime($d['date'])) ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>

        <?php else: ?>
            <p class="text-muted">Aucune dépense enregistrée aujourd'hui.</p>
        <?php endif; ?>
    </div>

</div>

</body>
</html>


